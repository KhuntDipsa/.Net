...
class Lab1
{
    ...
    static void Main(string[] args)
    {
        ...
        List<Employee> employees = new List<Employee>();                           // The array of Employee objects

        // Read the data file to build the Employee array and find out how many there are
        Read(employees, out count);
        ...
        while (loop)
        {
            ...
            if (Int32.TryParse(input, out choice))
            {
                switch (choice)
                {
                    ...
                    case 3:
                        employees.Sort((emp1, emp2) => emp1.Rate.CompareTo(emp2.Rate));
                        employees.Reverse();
                        break;

                    // Sort by weekly hours - descending
                    case 4:
                        employees.Sort((emp1, emp2) => emp1.Hours.CompareTo(emp2.Hours));
                        employees.Reverse();
                        break;

                    // Sort by gross pay - descending
                    case 5:
                        employees.Sort((emp1, emp2) => emp1.Gross.CompareTo(emp2.Gross));
                        employees.Reverse();
                        break;
    ...
    /*public static void Sort(List<Employee> emps, int size, int choice)
    {
            ...
            for (int b = 0; b < size - 1; b++)
                switch (choice)
                {
                    ...
                    case 2:  // Sort by employee ID - ascending
                        if (emps[b].Number > emps[b + 1].Number)
                            Swap(ref emps[b], ref emps[b + 1]);
                        break;

                    case 3:  // Sort by hourly rate -descending
                        if (emps[b].Rate< emps[b + 1].Rate)
                            Swap(ref emps[b], ref emps[b + 1]);
                        break;

                    case 4:  // Sort by weekly hours - descending
                        if (emps[b].Hours < emps[b + 1].Hours)
                            Swap(ref emps[b], ref emps[b + 1]);
                        break;

                    case 5:  // Sort by gross pay - descending
                        if (emps[b].Gross < emps[b + 1].Gross)
                            Swap(ref emps[b], ref emps[b + 1]);
                        break;
                }
    }*/
